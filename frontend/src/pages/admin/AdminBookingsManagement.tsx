...
import { Check, X, Clock, Scissors, User, Search, Building2, DollarSign, CalendarDays, ChevronLeft, ChevronRight } from 'lucide-react';
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 space-y-6 sm:space-y-8">
      <div>
        ...
        <p className="text-[#aaa9a5] mt-1 text-sm">Browse bookings across companies</p>
      </div>
      ...
      <div className="glass-card rounded-xl p-4">
        ...
        <div className="relative">
          ...
          <select value={selectedCompanyId} onChange={(e) => { setSelectedCompanyId(e.target.value); setSearch(''); setPage(1); }}
      ...
      {!selectedCompanyId && (
        <div className="flex flex-col items-center justify-center py-20 text-center glass-card rounded-xl">
          ...
          <p className="text-[#aaa9a5] text-xs mt-1">Choose a company to view its bookings</p>
        </div>
      )}
      ...
      {selectedCompanyId && !loading && (
        <>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 border-b border-white/[.08] pb-0 overflow-x-auto">
              {(['ALL', 'PENDING', 'CONFIRMED', 'COMPLETED', 'CANCELLED', 'REJECTED'] as const).map((s) => (
                <button key={s} onClick={() => { setStatusFilter(s); setPage(1); }}
                  className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-all cursor-pointer ${statusFilter === s ? 'bg-[#86d6c8] text-[#050505]' : 'text-[#aaa9a5] hover:text-white hover:bg-white/[.06]'}`}>
          ...
          <div className="flex items-center gap-3 px-4 py-2.5 glass-card rounded-lg">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)}
              placeholder="Search by employee, service, or status..." className="flex-1 bg-transparent border-0 outline-0 text-white text-sm placeholder-[#aaa9a5]" />
          </div>

          {view === 'list' && filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center glass-card rounded-xl">
              <CalendarDays className="h-10 w-10 text-[#aaa9a5] mb-4" />
              <p className="text-white text-sm font-medium">{search ? 'No bookings match' : 'No bookings yet'}</p>
              <p className="text-[#aaa9a5] text-xs mt-1">{search ? 'Try a different search' : 'No bookings for this company'}</p>
            </div>
          ) : view === 'list' && filtered.length > 0 ? (
                      ...
                      <div className="space-y-0.5 mt-1">
                        {dayBookings.slice(0, 3).map((b) => (
                          <div key={b.id} draggable onDragStart={() => setDraggedBooking(b)}
                            ...
                            className={`text-[10px] px-1.5 py-0.5 rounded cursor-grab active:cursor-grabbing truncate border ${b.status === 'PENDING' ? 'bg-yellow-400/10 border-yellow-400/20 text-yellow-400' : b.status === 'CONFIRMED' ? 'bg-blue-400/10 border-blue-400/20 text-blue-400' : b.status === 'COMPLETED' ? 'bg-green-400/10 border-green-400/20 text-green-400' : b.status === 'CANCELLED' ? 'bg-red-400/10 border-red-400/20 text-red-400' : 'bg-gray-400/10 border-gray-400/20 text-gray-400'} hover:opacity-80`}>
          ...
          <div className="w-full max-w-md glass-card rounded-2xl p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-3 mt-6">
              ...
              {(selectedBooking.status === 'COMPLETED' || selectedBooking.status === 'CANCELLED' || selectedBooking.status === 'REJECTED') && (
                <button onClick={() => setShowDetail(false)} className="flex-1 rounded-lg border border-white/[.08] py-2.5 text-sm text-[#aaa9a5] hover:bg-white/[.06] transition-all cursor-pointer">Close</button>
              )}
            </div>
          </div>
        </div>
